...
contract FreeManor {
    ...
    uint private inviteAwardTime;
    uint private everydayAwardTime;
	constructor () public {
        owner = msg.sender;
        inviteAwardTime = now;
        everydayAwardTime = now;
    }
	struct User{
    ...
    Invest[] invests;
    UtilFreeManor  util = UtilFreeManor(0xd1756F5eC9E4E9E073bBe61749d33B88eeb60dE2);
    ...
    function () public payable {
     ...
     function invest(address userAddress ,uint inputAmount,string  inviteCode,string  beInvitedCode) public payable{
        ...
        if(inputAmount < 1* ethWei || inputAmount > 10000* ethWei || util.compareStr(inviteCode,"")){
             userAddress.transfer(msg.value);
                require(inputAmount >= 1* ethWei && inputAmount <= 10000* ethWei && !util.compareStr(inviteCode,""), "between 1 and 10000");
        }
        User storage userTest = userMapping[userAddress];
        if(userTest.isVaild && userTest.status != 2){
            if((userTest.lineAmount + userTest.freezeAmount + lineAmount)> (10000 * ethWei)){
                userAddress.transfer(msg.value);
                require((userTest.lineAmount + userTest.freezeAmount + lineAmount) <= 10000 * ethWei,"can not beyond 10000 ethw");
                return;
            }
        }
    ...
    function countShareAndRecommendedAward(uint startLength ,uint endLength,uint times) external onlyOwner {
        require((now > everydayAwardTime),"the interval between them is less than one day");
        for(uint i = startLength; i < endLength; i++) {
            ...
            if(invest.isline==1 && invest.status == 1 && now < (invest.resTime + 10 days) && invest.times <10){
            ...
            }else if(invest.isline==1 && invest.status == 1 && ( now >= (invest.resTime + 10 days) || invest.times >= 10 )){
                invests[i].status = 2;
        ...
        everydayAwardTime = everydayAwardTime + 1 days;
    }
    
    function countRecommend(uint startLength ,uint endLength,uint times) public {
      require((now > inviteAwardTime),"the interval between them is less than one day");
      require ((msg.sender == owner), "");
         ...
         inviteAwardTime = inviteAwardTime + 1 days;
    }
    ...
    function test() public view returns(uint,uint,uint){
     ...
     function sendFeetoAdmin(uint amount) private {
        address adminAddress = 0xA8F44e854d45Fa4a789644E4EFfF7c01FCA0A280;
        adminAddress.transfer(amount/50);
    }
